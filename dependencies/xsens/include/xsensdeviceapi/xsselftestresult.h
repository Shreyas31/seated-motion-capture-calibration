...
struct XsSelfTestResult
{
	...
	/*!	\brief Returns whether the barometer passed (true) or failed (false) its self-test
		\details Only valid for MTi-7, 600-series
	...
	*/
	...
	/*! \brief Returns whether the gnss passed (true) or failed (false) its self-test
		\details Only valid for MTi-7, MTi-670, MTi-680
	...
	*/
	...
	/*! \brief Returns whether the fuelGauge passed (true) or failed (false) its self-test
		\details Only valid for DOT
		\returns True if the fuelGauge selftest has passed
	*/
	inline bool battery() const
	{
		return (m_flags & XSTF_Battery) != 0;
	}

	/*! \brief Returns whether the flash chip passed (true) or failed (false) its self-test
		\details Only valid for DOT, 600-series
		\returns True if the flash selftest has passed
	*/
	inline bool flash() const
	{
		return (m_flags & XSTF_Flash) != 0;
	}

	/*! \brief Returns whether the onboard button passed (true) or failed (false) its self-test
		\details Only valid for DOT
		\returns True if the button selftest has passed
	*/
	inline bool button() const
	{
		return (m_flags & XSTF_Button) != 0;
	}


	/*! \brief Returns whether the sync lines passed (true) or failed (false) its self-test
		\details Only valid for 600-series
		\returns True if the sync lines selftest has passed
	*/
	inline bool sync() const
	{
		return (m_flags & XSTF_Sync) != 0;
	}
#endif
};
